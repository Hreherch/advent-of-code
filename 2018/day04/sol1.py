...
while i+1 < len(logs):
    ...
    while i+1 < len(logs):
        i += 1
        if logs[i][5] == "up":
            t = timeBetween(logs[i-1], logs[i])[0]
            tracker[guard] = tracker.get(guard, 0) + t 
...
for guard in tracker:
    if tracker[guard] > howSleepy:
        mostSleepy = guard
        howSleepy = tracker[guard]

# find the sleepiest minute
i = 0
tracker = {} 
while i+1 < len(logs):
    while i+1 < len(logs) and logs[i][5] != mostSleepy:
        i += 1
    while i+1 < len(logs):
        i += 1
        if logs[i][5] == "up":
            mins = timeBetween(logs[i-1], logs[i])[1]
            for m in mins:
                tracker[m] = tracker.get(m, 0) + 1
        elif logs[i][5] != "asleep":
            break
