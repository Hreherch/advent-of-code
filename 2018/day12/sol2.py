...
# We only need to do a few runs to get the pattern
...
for i in range(n):
    g.live()
    # get the difference between the number of generations
    # and the value of the zero index
    delta = i + g.zeroIndex
# Since the zero index is always moving away to the left
# we can jump forward using the delta
value -= delta
v = 0
for p in g.garden:
    if p:
